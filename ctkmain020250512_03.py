...
# スプレッドシートIDと認証キーのパス
db_spreadsheet_id = '17Le1KA9nzMREt0Qp9_elM1OF1q8aSp-GDBZRPOntNI8'
...
worksheet = client.open_by_key(db_spreadsheet_id).sheet1
...
# ファイルクリア処理
 def clear_files():
    if messagebox.askyesno("確認", "本当にクリアして良いですか？"):
        try:
            for fname in ["checkd01.txt", "checkd02.txt", "output02.txt", "output.txt", "input.txt"]:
                if os.path.exists(fname):
                    open(fname, 'w', encoding='utf-8').close()
            messagebox.showinfo("クリア完了", "ファイルのデータをクリアしました。")
        except Exception as e:
            messagebox.showerror("エラー", f"ファイルをクリア中にエラー：{str(e)}")
...
def check_data_differences(output):
    try:
        ...
        messagebox.showerror("エラー", f"操作中にエラー：{str(e)}")
...
class MainWindow(customtkinter.CTk):
    ...
    def setup_ui(self):
        ...
        for text, cmd, r, c, col in buttons:
            btn = customtkinter.CTkButton(self.frame, text=text, command=cmd, fg_color=col)
            btn.grid(row=r, column=c, padx=5, pady=2, sticky="ew")
        for i in range(4): self.frame.columnconfigure(i, weight=1)
