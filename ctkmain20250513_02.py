...
def check_and_count_jan_codes(output):
    ...
    duplicate_jan_codes = [
        code for code, count in Counter(jan_codes).items() if count > 1
    ]
    ...
    # 「赤字＋太字」用タグ設定
    bold_font = tkfont.Font(font=output.cget("font"))
    bold_font.configure(weight="bold")
    output.tag_config("red_text", foreground="red", font=bold_font)
