...
def process_clipboard_data(jancode, clipboard_data):
    extracted_text = extract_jan_to_weight(clipboard_data)
    
    if extracted_text:
        formatted_text = format_extracted_text_for_input(extracted_text)
        with open('input.txt', 'a', encoding='utf-8') as file:
            file.write(formatted_text + '\n\n')
        print('JANコード～重量の抽出が完了し、input.txtに追加されました。')
        messagebox.showinfo("抽出・保存完了", f"JANコードから重量までのテキストを抽出し、input.txtに保存しました:\n\n{formatted_text}")
    else:
        ...
        print('通常のテキストがinput.txtに追加されました。')
...
def paste_and_execute():
    ...
    print(f"クリップボードから取得したテキスト: {clipboard_data}")
...
class MainWindow(customtkinter.CTk):
    ...
    def jancode_copy(self):
        """JANコード等のコピーボタンが押された時の処理 - 統合された機能の説明"""
        messagebox.showinfo(
            "JANコードコピー機能", 
            "JANコードコピー機能は「テキストに貼り付けと実行」ボタンに統合されました。\n\n"
            "使用方法:\n"
            "1. JANコードから重量までのテキストを選択してコピー\n"
            "2. 「テキストに貼り付けと実行」ボタンをクリック\n"
            "3. 自動的に抽出・フォーマットしてinput.txtに保存\n\n"
            "例: 「JANコード 4977292361613 ブランド名 ＳＫ１１ 商品名 電動サンダー用ナイロンタワシ 規格 #120 商品サイズ 幅95×高さ185×奥行き7mm 重量25ｇ」"
        )
        self.output.insert(customtkinter.END, "JANコード抽出機能は「テキストに貼り付けと実行」ボタンに統合されています。\n")
