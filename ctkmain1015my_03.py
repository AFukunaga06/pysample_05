...
def clear_files():
    if messagebox.askyesno("確認", "本当にクリアして良いですか？"):
        try:
            files_to_clear = ["checkd01.txt", "checkd02.txt", "output.txt", "input.txt"]
            for file in files_to_clear:
                with open(file, 'w', encoding='utf-8') as f:
                    f.write('')
            messagebox.showinfo("完了", "ファイルがクリアされました")
        except Exception as e:
            messagebox.showerror("エラー", f"ファイルのクリア中にエラーが発生しました: {str(e)}")
