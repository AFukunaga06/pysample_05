# -*- coding: utf-8 -*-
# Full script: 「テキストに貼り付けと実行」押下時に重複警告を出す版（個別ボタンで1回クリック起動・絶対パス解決）
...
def _resolve_path(path_like: str) -> str:
    """相対指定の場合はスクリプトのあるフォルダを優先して絶対パスに解決"""
    if os.path.isabs(path_like):
        return path_like
    cand1 = os.path.join(BASE_DIR, path_like)
    if os.path.exists(cand1):
        return cand1
    cand2 = os.path.join(os.getcwd(), path_like)
    if os.path.exists(cand2):
        return cand2
    return path_like  # 最後にそのまま返す（エラーメッセージ用）
...
#def open_file(file_path):
  ...
  #      return
    # .bat でも os.startfile でOK。二重起動を防ぐためクリックは1回想定
...
def open_file(file_path):
    ...
    # .batファイルの場合はsubprocessで実行
    if resolved.endswith('.bat'):
        try:
            subprocess.run([resolved], shell=True, check=True)
        except subprocess.CalledProcessError as e:
            messagebox.showerror("エラー", f"バッチファイルの実行に失敗しました: {str(e)}")
    else:
        os.startfile(resolved)
...
def execute_batch_and_open_file(batch_file, output_file):
    @safe_file_operation
    def execute():
        subprocess.run([_resolve_path(batch_file)])
        time.sleep(1)
...
def check_data_differences(output):
    try:
        with open('checkd01.txt', 'r', encoding='utf-8') as file1, open('checkd02.txt', 'r', encoding='utf-8') as file2:
            ...
            # checkd02.txt のJAN重複検出
            codes = []
            for line in lines2:
                m = re.search(r'\d+', line)  # 必要なら r'\d{13}' に変更
                if m:
                    codes.append(m.group())
            if any(cnt > 1 for cnt in Counter(codes).values()):
                messagebox.showwarning("警告", "JANコードが重複しています")
