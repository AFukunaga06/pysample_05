...
# -*- coding: utf-8 -*-
# Full script: テキスト貼り付け実行＋JAN重複警告＋checkd02/output.txt表示対応版（表示修正版）
...
def execute_batch_and_open_file(batch_file, output_file):
    ...
    def execute():
        ...
        if os.path.exists(output_file):
            ...
            window.output.insert(customtkinter.END, content)
        else:
            messagebox.showwarning("警告", f"{output_file} が見つかりません。")
...
class MainWindow(customtkinter.CTk):
    ...
    def setup_ui(self):
        ...
        buttons = [
            ...
            ("テキストに貼り付けと実行", paste_and_execute, 1, 2, "#008000"),
            ("座標軸とコピー", show_under_construction, 1, 3, "#FF0000"),

            ("データ抜けのチェック", lambda: execute_batch_and_skip_opening_file("Type1x.bat"), 2, 0, "#FF0000"),
            ("Type2x.bat実行とoutput.txt表示", lambda: execute_batch_and_open_file("Type2x.bat", "output.txt"), 2, 1, "#0078D4"),
            ("checkd01.txtを開く", lambda: open_file('checkd01.txt'), 2, 2, "#0078D4"),
            ("checkd02.txtを開く", lambda: self.show_file_content('checkd02.txt'), 2, 3, "#0078D4"),

            ("商品情報入力シートを開く", lambda: open_file('syouhin_n.bat'), 3, 0, "#0078D4"),
            ("藤原産業を開く", lambda: open_file('fujiwarasanngyou.bat'), 3, 1, "#0078D4"),
            ("チェックシートを開く", lambda: open_file(r'C:\pysample_01\checksheet12.bat'), 3, 2, "#0078D4"),
            ("サブフォーム廃番処理", self.open_subform, 3, 3, "#0078D4"),

            ("input.txtを開く", lambda: open_file('input.txt'), 4, 0, "#FF0000"),
            ("input.txtのチェック", self.check_input_file, 4, 1, "#0078D4"),
            ("クリップボードのクリア", clear_files, 4, 2, "#0078D4"),
        ]
        ...
        self.info_label.grid(row=5, column=0, columnspan=4, padx=5, pady=5, sticky="ew")
        ...
        self.info_label2.grid(row=6, column=0, columnspan=4, padx=5, pady=5, sticky="ew")
    ...
    def show_file_content(self, file_path):
        ...
        self.output.insert(customtkinter.END, content)
