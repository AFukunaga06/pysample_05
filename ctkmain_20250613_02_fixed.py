...
def get_data_differences_result():
    """データ差分チェックの結果を返す（現在は工事中）"""
    return "データ差分チェック機能は現在工事中です。\n"
...
class MainWindow(customtkinter.CTk):
    ...
    def jancode_copy(self):
        ...
        try:
            ...
            if jan_codes:
                latest_jan = jan_codes[-1]
                pyperclip.copy(latest_jan)
                messagebox.showinfo("コピー完了", f"JANコード {latest_jan} をクリップボードにコピーしました。")
            else:
                messagebox.showwarning("警告", "JANコードが見つかりませんでした。")
        except Exception as e:
